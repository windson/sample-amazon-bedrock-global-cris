#!/usr/bin/env python3
"""
Amazon Bedrock Global CRIS - Claude Fable 5 Pause After Compaction (InvokeModel)
...
def send_message(user_message: str, pause_on_compaction: bool = True):
    ...
    # Handle Fable 5 content restrictions (dual-use refusal)
    if stop_reason == "refusal":
        print(f"   ⚠️  Content restriction triggered (stop_reason: refusal)")
        messages.append({
            "role": "assistant",
            "content": model_response.get("content", [{"type": "text", "text": "[Refusal — prompt routed to Opus 4.8]"}])
        })
...
print("🌍 Amazon Bedrock Global CRIS - Claude Fable 5 Pause After Compaction")
...
try:
    ...
    prompt1 = f"Review this API specification and analyze the architecture patterns and suggest scalability improvements:\n\n{spec1}"
    ...
    if was_paused:
        ...
        reminder = (
            ...
            "N+1 query risks in include parameters, missing HTTP caching, "
            "and unbounded pagination as key issues. Keep these in mind "
